...
def kICSSlideWinFn(s, kSqGrid, tauGrid):
    ...
    diff_term = (A ** (-2) * (A+s['K']) ** (-2) * s['r'] * (np.exp((-1) * A * ((-1)+tauGrid)) * (1+np.exp((-2) * A)+(-2) * np.exp((-1) * A)) * s['K'] * (2 * A+s['K']) * s['r']+(-1) * A ** 2 * np.exp(((-1) * A+(-1) * s['K']) * ((-1)+tauGrid)) * ((1+(-1) * np.exp((-1) * A + (-1) * s['K'])) ** 2 * ((-1)+s['r'])+(-1) * np.exp((-1) * s['K'] * (1+(-1) * tauGrid)) * (1+(-1) * np.exp((-1) * A)) ** 2 * s['r'])+(-1) * (np.exp((-1) * A * ((-1)+tauGrid))*(1 + np.exp((-2) * A)) * s['K'] * (2 * A+s['K']) * s['r']+(-1) * np.exp((-1) * A * ((-2)+tauGrid)+(-1) * s['K'] * ((-1)+tauGrid)) * (A ** 2 * np.exp((-1) * A) * (1+(-1) * np.exp((-1) * A+(-1) * s['K'])) * (1+(-1) * np.exp(((-1) * A+(-1) * s['K']) * s['Ts']))*((-1)+s['r']) + np.exp((-1) * A+(-1) * s['K'] * (1+(-1) * tauGrid)) * ((-1) * A ** 2+np.exp((-1) * A) * (A+s['K']) ** 2 + np.exp((-1) * A * s['Ts']) * (1+(-1) * np.exp((-1) * A)) * (A+s['K']) ** 2 + np.exp((-2) * A) * s['K'] * (2 * A+s['K'])) * s['r'])) * s['Ts'] ** (-1)+((-2) * A ** 3 + np.exp((-1) * A * tauGrid) * (1+(-1) * np.exp((-1) * A))*(1 + np.exp((-1) * A * ((-1)+s['Ts']+(-2) * tauGrid))) * s['K'] ** 2 * s['r']+(-2) * A * s['K'] * ((-1) * np.exp((-1) * A * ((-1)+s['Ts']+(-1) * tauGrid)) + np.exp((-1) * A * (s['Ts']+(-1) * tauGrid))+(-1) * np.exp((-1) * A * tauGrid) + np.exp((-1) * A * (1+tauGrid))+s['K']) * s['r']+A ** 2 * ((-1) * np.exp(((-1) * A+(-1) * s['K']) * ((-1)+s['Ts']+(-1) * tauGrid)) * ((-1)+s['r']) + np.exp(((-1) * A+(-1) * s['K']) * (s['Ts']+(-1) * tauGrid)) * ((-1)+s['r'])+(-1) * np.exp(((-1) * A+(-1) * s['K']) * tauGrid) * ((-1)+s['r']) + np.exp(((-1) * A+(-1) * s['K']) * (1+tauGrid)) * ((-1)+s['r']) + np.exp((-1) * A * ((-1)+s['Ts']+(-1) * tauGrid)) * s['r']+(-1) * np.exp((-1) * A * (s['Ts']+(-1) * tauGrid)) * s['r'] + np.exp((-1) * A * tauGrid) * s['r']+(-1) * np.exp((-1) * A * (1+tauGrid)) * s['r']+(-2) * s['K'] * (1+s['r']))) * s['Ts'] ** (-1)+ \
                 s['Ts'] ** (-2) * ((-1) * np.exp((-1) * A * ((-1)+tauGrid)) * (A ** 2 * np.exp((-1) * s['K'] * ((-1)+tauGrid)) * (1+(-2) * np.exp((-1) * A+(-1) * s['K']) + np.exp((-2) * (A+s['K'])))*((-1)+s['r'])+(-1) * (1 + np.exp((-2) * A)+(-2) * np.exp((-1) * A)) * (A+s['K']) ** 2 * s['r']) * s['Ts'] + np.exp((-1) * A * tauGrid) * ((A+s['K']) ** 2 * s['r'] * ((-1) + np.exp((-1) * A * s['Ts'])+s['Ts']+(-1) * np.exp((-1) * A) * s['Ts'])+A ** 2 * np.exp((-1) * s['K'] * tauGrid) * ((-1)+s['r']) * (1+(-1) * np.exp(((-1) * A+(-1) * s['K']) * s['Ts'])+(-1) * s['Ts'] + np.exp((-1) * A+(-1) * s['K']) * s['Ts']))+2 * A * (A+s['K']) * (A+A * (1+(-1) * np.exp((-1) * A+(-1) * s['K'])) * (A+s['K']) ** (-1) * ((-1)+s['r'])+((-1) + np.exp((-1) * A))*s['r']+A ** (-1) * ((-1)+A + np.exp((-1) * A)) * s['K'] * s['r']) * (s['Ts']+(-1) * tauGrid)+2 * A * s['K'] * s['r'] * ((-1) + np.exp((-1) * A * (s['Ts']+(-1) * tauGrid))+s['Ts']+(-1) * np.exp((-1) * A) * (s['Ts']+(-1) * tauGrid)+(-1) * tauGrid)+s['K'] ** 2 * s['r'] * ((-1) + np.exp((-1) * A * (s['Ts']+(-1) * tauGrid))+s['Ts']+(-1) * np.exp((-1) * A) * (s['Ts']+(-1) * tauGrid)+(-1) * tauGrid)+A ** 2 * ((-1)+(-1) * np.exp(((-1) * A+(-1) * s['K']) * (s['Ts']+(-1) * tauGrid)) * ((-1)+s['r']) + np.exp((-1) * A * (s['Ts']+(-1) * tauGrid)) * s['r']+s['Ts'] + np.exp((-1)*A+(-1) * s['K']) * ((-1)+s['r']) * (s['Ts']+(-1) * tauGrid)+(-1) * np.exp((-1) * A) * s['r'] * (s['Ts']+(-1) * tauGrid)+(-1) * tauGrid)+(-1) * (A+s['K']) ** 2 * s['r'] * ((-1)+(-1) * np.exp((-1) * A * tauGrid) * ((-1)+s['Ts'])+(-1) * s['Ts'] + np.exp((-1) * A * ((-1)+tauGrid)) * s['Ts'] + np.exp((-1) * A) * (s['Ts']+(-1) * tauGrid)+tauGrid)+A ** 2 * ((-1)+s['r']) * ((-1)+(-1) * np.exp(((-1) * A+(-1) * s['K']) * tauGrid) * ((-1)+s['Ts'])+(-1) * s['Ts'] + np.exp(((-1) * A+(-1) * s['K']) * ((-1)+tauGrid)) * s['Ts'] + np.exp((-1) * A+(-1) * s['K']) * (s['Ts']+(-1) * tauGrid)+tauGrid))))
    ...
    diff_term = (A ** (-2) * (A+s['K']) ** (-2) * s['r'] *(np.exp((-1) * A * ((-1)+tauGrid)) * (1+np.exp(
                ...
                *A+(-1) *s['K'])) *(1+(-1) * np.exp(((-1) *A+(-1) *s['K']) *s['Ts'])) 
                ...
                s['r']+A **2 *((-1) * np.exp(((-1) *A+(-1) *s['K']) *((-1)+s['Ts']+(-1) *tauGrid)) *((  
                 ...
                 * np.exp((-1) *A *(1+tauGrid)) *s['r']+(-2) *s['K'] *(1+s['r']))) *s['Ts'] **(-1)+s['Ts'] **(-2)  
                 *((-1) * np.exp((-1) *A *((-1)+tauGrid)) *(A **2 * np.exp((-1) *s['K'] *((  
