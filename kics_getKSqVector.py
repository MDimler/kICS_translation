...
def getKSqVector(r_k, varargin, nargout = False):
    ...
    for i in range(len(varargin)):
        if varargin[i].lower() == 'kSqMin'.lower():
            ...
            if varargin[i+1].isnumeric() and varargin[i+1] in (0,1):
    ...
    if ksq_max.lower() == 'max'.lower():
        iksq_max = len(ksq) 
        ksq_max = ksq[-1]    
    else:
        for j in range(len(ksq),0,-1):  # Exchanged for iksq_max = find(ksq <= ksq_max,1,'last');
            if ksq[j] <= ksq_max:
                iksq_min = j
                break
    ...
    if nargout:
        ...
        lattice_inds = np.where(k_lattice_sqrd >= ksq_min and k_lattice_sqrd <= ksq_max)
    ...
    if nargout:
        return (ksq_sub, iksq_sub, lattice_inds)
    else:
